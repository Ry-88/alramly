...
<div class="container" style="margin-top:20px">
    <h3 style="margin-bottom: 5rem;">الأماكن</h3>
       <form class="form form-vertical" action="{{ route('texts.update',$text->id) }}" method="POST" enctype="multipart/form-data">
           @csrf
              @method('PUT')
           <div class="row">
               <div class="col-8">
                   <div class="form-group">
                       <label for="first-name-vertical">أسم النص</label>
                       <input type="text" id="first-name-vertical" class="form-control" name="title" value="{{ $text->title }}">
                       @error('title') 
               ...
               <div class="col-8">
                <div class="form-group">
                    <label for="email-id-vertical">النص</label>
                       <textarea id="editor"  class="form-control" name="body" id="email-id-vertical" rows="3">
                           {{ $text->body }}
                       </textarea>
                    @error('body') 
                    <span id="basic-default-name-error" class="error">
