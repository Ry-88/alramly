...
<div class="container" style="margin-top:20px">
    ...
    <div class="row">
        ...
        <div class="col-6">
            <a href="{{ route('news.index') }}" class="btn btn-outline-success waves-effect">رجوع</a>
        </div>
    </div>
       <form class="form form-vertical" action="{{ route('news.update',$news->id) }}" method="POST" enctype="multipart/form-data">
           @csrf
              @method('PUT')
           <div class="row">
               <div class="col-8">
                   <div class="form-group">
                       <label for="first-name-vertical">عنوان الخبر بالعربي</label>
                       <input type="text" id="first-name-vertical" class="form-control" name="title" value="{{ $news->getTranslations('title')['ar'] }}">
                       @error('title') 
               ...
               <div class="col-8">
                <div class="form-group">
                    <label for="first-name-vertical"> عنوان الخبر بالانجليزي</label>
                    <input type="text" id="first-name-vertical" class="form-control" name="title_en" value="{{ $news->getTranslations('title')['en'] }}">
                    @error('title_en') 
                    <span id="basic-default-name-error" class="error">
                        {{ $message }}
                    </span>
                ...
                <div class="form-group">
                    <label for="email-id-vertical">المحتوى بالعربي</label>
                       <textarea id="editor"  class="form-control" name="content" id="email-id-vertical" rows="3">
                        {{ $news->getTranslations('content')['ar'] }}
                       </textarea>
                    @error('content') 
                    <span id="basic-default-name-error" class="error">
                        {{ $message }}
                ...
                <div class="form-group">
                    <label for="email-id-vertical">المحتوى بالانجليزي</label>
                       <textarea id="editorEn" class="form-control" name="content_en" id="email-id-vertical" rows="3">
                        {{ $news->getTranslations('content')['en'] }}
                       </textarea>
                    @error('content_en') 
                    <span id="basic-default-name-error" class="error">
                        {{ $message }}
